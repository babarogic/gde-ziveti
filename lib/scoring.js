...
const PHASE_VALUE = { good: 5, neutral: 3, bad: 1 };

/** Ocena lokacije za izabranu fazu života, na skali 1–5. */
export function phaseScore(locId, phaseId) {
  const phase = PHASES.find(item => item.id === phaseId) || PHASES[0];
  const items = phase?.locs?.[locId] || [];
  if (!items.length) return 3;
  return items.reduce((sum, [status]) => sum + PHASE_VALUE[status], 0) / items.length;
}
...
export function rankLocations({
  goranPrio, partnerPrio, goranRating, partnerRating,
  goranDB, partnerDB, fit, dbStatus, activePhase = 'now',
}) {
  return LOCATIONS.map(loc => {
    const gGut = goranRating?.[loc.id] || 0;
    const pGut = partnerRating?.[loc.id] || 0;
    const gBase = weightedScore(goranPrio, fit, loc.id);
    const pBase = weightedScore(partnerPrio, fit, loc.id);
    const phaseFit = phaseScore(loc.id, activePhase);
    const gCalc = gBase ? (gBase * 0.8) + (phaseFit * 0.2) : 0;
    const pCalc = pBase ? (pBase * 0.8) + (phaseFit * 0.2) : 0;
